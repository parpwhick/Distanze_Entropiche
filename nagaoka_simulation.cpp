...
double nagaoka_simulation::kinetic_energy() {
    double totale = 0.0;
...
double nagaoka_simulation::magnetic_energy() {
    double dH=0;
...
double nagaoka_simulation::average_position(){
    int side = opts.lato;
    double position=0.0;
    if(hamiltonian.ground_state.empty() || nonquantum)
        return NAN;
...
template <typename T> void shift_lattice(vector<T> &conf, int by, int rows, int cols) {
    ...
    for (int from = 0; from < cols; from++) {
        //destination column
        int to = (from + by + 2 * cols) % cols;
        for (int i = 0; i < rows; i++)
            conf[to * rows + i] = temp[from * rows + i];
    }
}
...
void nagaoka_run(const adj_struct &adj) {
    ...
    //thermalize before the main run
    sim.nonquantum = true;
    sim.step_wh(opts.skip * 100);
    sim.nonquantum = true;
    //thermalize with the polaron
    sim.step_wh(opts.skip);
    sim.hamiltonian.set_confining(0);
    sim.hamiltonian.set_V(V);
    shifts = 0;

    if (opts.restart && opts.dynamics == METROPOLIS) {
...
restarting:
        //if the polaron has gone too far, restart
        if (!config_backup.empty() && (new_position > 0.75 * L || new_position < 0.25 * L)) {
            displacement = known_position - new_position;
            sim.config = config_backup;
            sim.link_energies = energy_backup;
            sim.hamiltonian.ground_state = groundstate_backup;
            sim.hamiltonian.set_V(V);
            new_position = known_position;
        }
        else 
            displacement = 0;
        //if the polaron is around the middle, save a snapshot
        if (//opts.dynamics != METROPOLIS && 
                abs(new_position - 0.5 * L) < 0.3) {
            config_backup = sim.config;
            energy_backup = sim.link_energies;
            groundstate_backup = sim.hamiltonian.ground_state;
...
circular_shift:
        if ((new_position > (0.75 * L)) || (new_position < (0.25 * L))) {
            ...
            shift_lattice(sim.config, displacement, L, L);
            shift_lattice(sim.hamiltonian.ground_state, displacement, L, L);
            if(opts.dynamics != METROPOLIS)
                shift_lattice(sim.link_energies, displacement, 2 * L, L);
            sim.hamiltonian.lanczos_lowest_energy();
...
printstuff:
        ...
        E_kin = sim.kinetic_energy();
        E_mag = - sim.magnetic_energy();
        ...
        if (opts.verbose > 1) {
            ...
            if (opts.verbose > 2) {
                ...
                if (opts.dynamics != METROPOLIS){
                    ...
                    write_binary_array(sim.link_energies.data(), sim.link_energies.size(), ("link_energies" + opts.suffix_out + ".bin"), "ab");
                }
            }
        }
        if (opts.verbose)
