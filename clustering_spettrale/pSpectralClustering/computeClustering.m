...
% by recursively computing bipartitions using the second eigenvector of the p-Laplacian.
%
...
%
% Written by Thomas B�hler and Matthias Hein
...
% http://www.ml.uni-saarland.de
    ...
    if (successive)
        ...
        [vmin,fmin,umin,normGrad] = minimizeFunctional(W,p,1000000,start,normalized,false);
    end
            ...
            if (~isempty(subClusters{m}))
				...
				%	fprintf('Computing partitioning of subgraph %d for p=%.3f.\n',m,p);
					if (successive)
						...
						vminM = minimizeFunctional(Wm,p,1000000,startM,epsilon,normalized,false);
					end
				...
				%	fprintf('...Computing cluster indicator function.\n\n');
					if threshold_type>=0
						...
						allClustersInClusterM= computeClusterIndicatorFunction(vminM,threshold);
						...
						allClustersInClusterM= computeClusterIndicatorFunction(vminM,vminM_sorted(threshold_index)); % 1 if vmin> threshold
